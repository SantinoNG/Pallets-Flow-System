        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-b border-gray-200">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-6">
                    Editando: <span class="text-indigo-600">{{ $employee->first_name }} {{ $employee->last_name }}</span>
                </h3>
                        ...
                        <select id="id_user" name="id_user"
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id_user }}"
                                    {{ old('id_user', $employee->id_user) == $user->id_user ? 'selected' : '' }}>
                                    {{ $user->email }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="id_role" name="id_role"
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id_role }}"
                                    {{ old('id_role', $employee->id_role) == $role->id_role ? 'selected' : '' }}>
                                    {{ $role->role_name }}
                                </option>
                            @endforeach
                        </select>
